...
- **Toekomstige uitbreiding: zelfstandige woonruimte (WWS, niet WWSO)** — idee van de gebruiker (2026-09-04): de app zou op termijn ook zelfstandige woningen moeten kunnen doorrekenen, niet alleen onzelfstandige verhuur. Bewust laag geprioriteerd: pas ná de eerste bèta-release oppakken, niet nu — blijft on hold achter hetzelfde tussenfase-exitcriterium als Fase 4. **Bijgewerkt (2026-09-07)**: het aparte WWS-beleidsboek is inmiddels in huis (`resources/Beleidsboek zelfstandig/`), en op verzoek van de gebruiker alvast uitgewerkt tot een volledige, ongestarte **Fase 5** in `plan/plan.md` (taak 24-30) — architectuurkeuze: twee gescheiden invoerflows/menu-opties (WWS kent geen kamertoewijzing, dus het bestaande invoerscherm is structureel ongeschikt), met zoveel mogelijk gedeelde rekenregels onder de motorkap. Eerste steekproef bevestigt dat de huurprijs-lookup-mechaniek (punten→tabel, >250pt-extrapolatie) al generiek genoeg is in de bestaande `bepaalMaxHuur()` om over te nemen. Puur een plan, geen code gewijzigd.
- **Roadmap-items op verzoek van de gebruiker toegevoegd (2026-09-05): code audit, security, persoonsgegevens/AVG, PSP (betaaldienstverlener).** Fasering bevestigd door de gebruiker (2026-09-05):
  - **Vóór de bèta**: het essentiële beveiligingswerk dat al gepland stond — auth-toggle dicht + RLS aan (zie bovenaan deze lijst, nog open). **Lichte privacy-check van de vrije-tekstvelden: afgerond (2026-09-20).** Alle drie de vrije-tekstvelden gecontroleerd — `deals.notitie`/`.map` (21 woningen, via de anon-API tijdens de open-toggle-periode) en `feedback.bericht` (via SQL-editor, RLS-beschermd). Geen huurder-/derdengegevens gevonden — alleen testartefacten en de voornamen van de drie bekende testers in het `map`-veld (zelf-organisatie van ingelogde gebruikers, geen huurdersdata, dus buiten de "geen persoonsgegevens"-beslissing). Geen wijzigingen nodig.
...
testen — de lichte privacy-check is inmiddels afgerond (zie hierboven), geen bevindingen.
