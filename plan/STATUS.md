...
- Vitest, ESLint, Prettier op root- en packageniveau — 259/259 tests groen
...
- **Bèta-lancering met drie testers (nieuw plan, 2026-09-04)**: de gebruiker wil bij een volgende go-live ook gescheiden org_id's zetten om een bèta met drie testers te kunnen draaien, maar denkt zelf dat vrije notities en "mappen" (nieuw begrip, nog niet eerder genoemd — vermoedelijk een manier om deals te groeperen, bijv. per tester/klant) er eerst moeten zijn. Volgorde-inzicht van de gebruiker zelf, nog niet verder uitgewerkt: (1) notities + mappen bouwen, (2) multi-tenant/org_id's scheiden, (3) bèta lanceren. Concrete scope van "mappen" nog te bepalen — is dit een nieuwe deals-groepering, of iets anders? Eigen domein is inmiddels beschikbaar (zie de eigen-SMTP-regel in `plan/plan.md`), dus de mail-rate-limit hoeft geen blokkade meer te zijn bij drie testers.
- Vrije notities bij een deal/pand (idee van de gebruiker, 2026-09-01, nu ook onderdeel van de bèta-lancering hierboven) — scope nog te bepalen: los tekstveld of gestructureerd, per deal of scenario, waar zichtbaar.
- "Mappen" bij deals (idee van de gebruiker, 2026-09-04, genoemd i.c.m. de bèta-lancering) — scope nog volledig onduidelijk, eerst uitvragen wat hiermee bedoeld wordt.
...
- Multi-tenant/gescheiden org_id's — nu concreter dan "erkende komende taak": de gebruiker wil dit expliciet vóór een bèta-lancering met drie testers (zie hierboven), niet meer alleen bij een trigger vanuit Steven.
...
- Nieuw plan van de gebruiker voor een bèta-lancering met drie testers: eerst notities + "mappen" bouwen, dan multi-tenant/org_id's scheiden, dan lanceren (zie "Openstaande beslissingen"). De gebruiker heeft inmiddels een eigen domein, wat de eigen-SMTP-blokkade voor magic-link-mails opheft.

## Volgende concrete actie
Gedeployed naar productie (2026-09-04, laatste versie 0.5.0, `vercel deploy --prod` — `vercel --prod` zelf gaf een "Not authorized"-fout, de CLI's eigen aanbevolen `vercel deploy --prod` werkte wel). Geen van de tussenfase-taken of de COROP-automatisering is nog opgehangen aan een openstaande browsercontrole. Eerstvolgende stap: samen met de gebruiker de bèta-lancering hierboven verder uitwerken (scope van "mappen" bepalen, notities scopen, dan multi-tenant). Daarna, of ondertussen: taak B (kitchenette-varianten, wacht nog op Stevens prijzensheet) — de enige resterende tussenfase-taak. Taak 18 en de rest van Fase 4 blijven on hold tot het tussenfase-exit-criterium gehaald is (zie hierboven). Standaard Sonnet — geen van de vier Opus-triggers is hier van toepassing.
