...
- [ ] Invoerscherm (`/pand/nieuw`) slaat tussentijds niets op: alleen React-state (`useReducer` in `InvoerContext.tsx`), dus browser terug/refresh/tabblad sluiten tijdens het invullen verliest alles. Pas bij "Doorrekenen" (naar sessionStorage) of expliciet "Deal opslaan" (naar Supabase) wordt er iets bewaard. Fix: dezelfde soort sessionStorage-autosave als de bestaande brug, maar dan al tijdens het typen, met hervatten bij het laden van `/pand/nieuw`. Gevonden tijdens de eerste live test van de Vercel-deploy, 2026-08-20.
...
- [ ] De as-is invoer van een opgeslagen deal kan achteraf niet aangepast worden — "Deal opslaan"/"Deal bijwerken" op het vergelijkingsscherm slaat alleen de scenario-keuzes opnieuw op, de pandgegevens zelf zijn daar read-only. Nodig voor het geval dat je na een bezichtiging/meer informatie de as-is wilt bijstellen zonder een nieuwe deal te beginnen. Fix: een weg van een geladen deal terug naar het bewerkbare invoerformulier (`/pand/nieuw`), met de deal-id meegenomen zodat "Doorrekenen" dezelfde deal bijwerkt in plaats van een nieuwe aan te maken. Gevonden 2026-08-20 tijdens feedback-ronde na de eerste deploy.
